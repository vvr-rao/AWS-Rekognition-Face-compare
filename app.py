...
def compare_faces(bucket, key):
    response = rekognition.compare_faces( SourceImage={"S3Object": {"Bucket": "MYBUCKET", "Name": "template.jpg"}} , 
    TargetImage={"S3Object": {"Bucket": bucket, "Name": key}} )
    numfaces = 0
    for label_prediction in response['FaceMatches']:
        numfaces = numfaces + 1
        response_string = 'Matched face with similarity {}'.format(label_prediction['Similarity'])
        print(response_string)
    for label_prediction in response['UnmatchedFaces']:
        numfaces = numfaces + 1
        response_string = 'UnMatched face with confidence {}'.format(label_prediction['Confidence'])
        print(response_string)
    print('number of faces detected {}'.format(numfaces))
    return response
